...
type PreviewKind = 'portada' | 'historia'
...
function buildPortadaClinicaPdf(portada: PortadaEntry) {
  ...
  doc.setDrawColor(0)
  doc.rect(boxX, y - 8, boxW, 18)
  doc.setFont('helvetica', 'bold')
  doc.setFontSize(9.5)
  doc.text('N° SOCIO', boxX + boxW / 2, y - 2, { align: 'center' })
  doc.setFont('helvetica', 'normal')
  doc.setFontSize(11)
  doc.text(portada.nroSocio?.trim() || '—', boxX + boxW / 2, y + 5.2, { align: 'center' })

  doc.rect(boxX, y + 10, boxW, 18)
  doc.setFont('helvetica', 'bold')
  doc.setFontSize(9.5)
  doc.text('DNI', boxX + boxW / 2, y + 16, { align: 'center' })
  doc.setFont('helvetica', 'normal')
  doc.setFontSize(11)
  doc.text(portada.dni?.trim() || '—', boxX + boxW / 2, y + 23.2, { align: 'center' })

  doc.setFont('helvetica', 'bold')
  doc.setFontSize(12)
  const nameLines = doc.splitTextToSize((portada.apellidoNombre || ' ').toUpperCase(), contentW - boxW - 10)
  doc.text(nameLines, margin, y)
  y += nameLines.length * 6 + 8

  y += drawLabelValue('DOMICILIO', portada.domicilio?.trim() || '—', margin, y, contentW)
  y += drawLabelValue('FECHA DE NAC.', fmtFechaNacimiento(portada.fechaNacimiento || '') || '—', margin, y, contentW)

  doc.setDrawColor(210)
  doc.line(margin, pageH - 14, pageW - margin, pageH - 14)
  doc.setTextColor(110)
  doc.setFontSize(8.6)
  doc.text('Medic Laboral - Portada clínica', margin, pageH - 8)
...
function buildHistoriaClinicaPdf(payload: HistoriaClinicaPdfPayload) {
  ...
  const firmaY = pageH - 42
  doc.setDrawColor(120)
  doc.line(margin + 4, firmaY, margin + 72, firmaY)
  doc.line(pageW - margin - 72, firmaY, pageW - margin - 4, firmaY)
  doc.setFont('helvetica', 'normal')
  doc.setFontSize(9)
  doc.setTextColor(0)
  doc.text('FIRMA DEL TRABAJADOR', margin + 38, firmaY + 6, { align: 'center' })
  doc.text('FIRMA DEL MÉDICO', pageW - margin - 38, firmaY + 6, { align: 'center' })
...
export default function PortadaScreen() {
  ...
  const [previewKind, setPreviewKind] = useState<PreviewKind>('portada')
...
setPadronByCompanyId(new Map<string, CompanyPadronPerson[]>(padronEntries))
  ...
  function handlePreviewPortada() {
    if (!selected) return
    setPreviewOpen((prev) => {
      const next = !prev || previewKind !== 'portada'
      if (next) {
        setPreviewKind('portada')
        const doc = buildPortadaClinicaPdf(selected)
        setPdfPreview(doc)
      } else {
        closePreview()
      }
  ...
  function handleDownloadPortada() {
    if (!selected) return
    const doc = buildPortadaClinicaPdf(selected)
    const namePart = safeFilePart(selected.apellidoNombre || 'portada_clinica')
    doc.save(`Portada_Clinica_${namePart}_${new Date().toISOString().slice(0, 10)}.pdf`)
  }

  function handlePreviewHistoria() {
    if (!pdfPayload) return
    setPreviewOpen((prev) => {
      const next = !prev || previewKind !== 'historia'
      if (next) {
        setPreviewKind('historia')
  ...
  function handleDownloadHistoria() {
    if (!pdfPayload) return
    const doc = buildHistoriaClinicaPdf(pdfPayload)
  ...
  useEffect(() => {
    ...
    return () => window.clearTimeout(t)
  }, [pdfPayload, previewKind, previewOpen, selected])
      ...
      <div className="card portada__card">
        <div className="portada__hist-head">
          ...
          <div className="portada__actions">
            <button className="btn btn--outline" type="button" onClick={handlePreviewPortada} disabled={!selected}>
              {previewOpen && previewKind === 'portada' ? 'Cerrar previsualización' : 'Previsualizar PDF'}
            </button>
            <button className="btn btn--primary" type="button" onClick={handleDownloadPortada} disabled={!selected}>
              Descargar PDF
            </button>
                ...
                <>
                  <div className="detail-panel__head">
                    ...
                    <div className="detail-panel__actions">
                      <button className="btn btn--outline btn--sm" type="button" onClick={handlePreviewHistoria}>
                        Previsualizar
                      </button>
                      <button className="btn btn--primary btn--sm" type="button" onClick={handleDownloadHistoria}>
                        Descargar PDF
                      </button>
                    </div>
                  </div>
