...
export default function PreocupacionalScreen() {
  ...
  const pdfDateText = useMemo(() => fmtDateFromISO(draft.fechaTurnoISO) || today, [draft.fechaTurnoISO, today])
  ...
  const currentDocCompleto = useCallback(async () => buildPdfCompleto(draft, pdfDateText), [draft, pdfDateText])
  const currentDocVista = useCallback(async () => buildPdfVista(view, draft, pdfDateText), [view, draft, pdfDateText])
  ...
  const coercePrefill = useCallback((state: unknown): PreocupacionalPrefill | null => {
    ...
    return {
      ...
      fechaTurnoISO: maybe.turno.fechaTurnoISO || '',
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      ...
      setDraft(() => ({
        ...
        fechaTurnoISO: pick(payload.fechaTurnoISO, ''),
        checks: nextChecks,
