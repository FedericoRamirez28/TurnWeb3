...
export async function authMe(): Promise<AuthUser | null> {
  const r = await fetchJSON<{ user: AuthUser | null }>(`${API_BASE_URL}/auth/me`)
  syncDevUserId(r.user)
  return r.user
}

export async function authLogin(username: string, password: string, remember: boolean): Promise<AuthUser> {
  const r = await fetchJSON<{ user: AuthUser }>(`${API_BASE_URL}/auth/login`, {
    method: 'POST',
...
export async function authRegister(
  ...
  const r = await fetchJSON<{ user: AuthUser }>(`${API_BASE_URL}/auth/register`, {
    method: 'POST',
...
export async function requestPasswordReset(username: string): Promise<string> {
  const r = await fetchJSON<{ ok: true; token: string }>(`${API_BASE_URL}/auth/password/request`, {
    method: 'POST',
